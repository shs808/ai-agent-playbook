...
OpenCode를 쓸 때 “어떤 모델을 기본으로 쓸지”를 하나로 고정하기보다,  
**작업 종류에 따라 잘 맞는 모델을 자동 선택**하도록 구성한 설정입니다.
...
아래 프롬프트를 그대로 붙여 넣으면, 과한 감사 절차 없이 적용 계획을 만들도록 요청할 수 있습니다.
...
한 줄 결론
이 설정은 “무조건 가장 강한 모델 하나”를 쓰는 방식이 아니라,  
작업에 맞는 모델을 먼저 쓰고 문제가 있을 때만 비슷한 급의 모델로 한 번 대체하는 실용적인 OpenCode 프리셋입니다.
