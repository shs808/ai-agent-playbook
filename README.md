...
GPT 5.6 기반으로 **작업 종류에 따라 잘 맞는 모델을 선택**하도록 구성한 설정입니다.

예를 들면 일반 구현은 Sol, UI 작업은 Terra, 빠른 탐색은 Mini 모델을 쓰는 식입니다.

## 3줄 요약

1. OpenCode 1.18.x의 GPT-5.5 중심 설정을 GPT-5.6 계열의 작업별 모델 라우팅으로 전환합니다.  
2. 복잡한 작업은 Sol, 일반·UI 작업은 Terra, 가벼운 작업은 Luna·Mini에 맡겨 품질과 비용을 함께 관리합니다.  
3. 기본 모델 장애 시 지정한 fallback을 한 번만 사용하며, 재시작 후 새 Prometheus 세션으로 적용 여부를 확인합니다.
...
## Agent 설정을 위해 OpenCode에 넣을 프롬프트
